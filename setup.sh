...
sudo useradd ddclient -u $DDCLIENT_UID
sudo useradd cloudflared -u $CLOUDFLARED_UID
...
sudo usermod -a -G mediacenter ddclient
sudo usermod -a -G mediacenter cloudflared
...
sudo mkdir -pv ${ROOT_DIR:-.}/config/{sonarr,radarr,lidarr,readarr,mylar,prowlarr,qbittorrent,jackett,audiobookshelf,overseerr,plex,jellyfin,tautulli,sabnzbd,jellyseerr,bazarr,swag,authelia,homepage,homeassistant,wireguard,glances,gluetun,speedtest,ddclient,cloudflared}-config
